...
test('[P0] signed-out onboarding keeps Local Agent and BYOK available without Cloud authorization', async ({ page }) => {
  ...
  // Cloud remains the primary action, while account-free runtimes stay
  // available before authentication resolves.
  const primary = cloudPrimaryButton(page);
  await expect(primary).toBeVisible();
  await expect(primary).toHaveText(/Sign in to OpenDesign|登录 OpenDesign/i);
...
test('[P0] signed-out users are redirected from Home to onboarding', async ({ page }) => {
  const config = await wireOnboardingMocks(page, {
    amrAvailable: true,
    initialLoggedIn: false,
    keepAmrLoginIncomplete: true,
...
for (const destination of [
  ...
  test(`[P0] signed-out users are redirected from ${destination.name} to onboarding`, async ({ page }) => {
    const config = await wireOnboardingMocks(page, {
      amrAvailable: true,
      initialLoggedIn: false,
      keepAmrLoginIncomplete: true,
